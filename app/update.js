...
export const startup = () => {
  ...
  .then((ready) => {
    if (ready) {
      remove(join(basedir, 'legend'))
      .catch(err => null)
      .then(() => move(join(basedir, 'old'), join(basedir, 'legend'))
      .catch(err => null)
      .then(() => ensureDir('old'))
      .then(() => rename(join(basedir, 'app'), join(basedir, 'old', 'app'))
      .then(() => rename(join(basedir, 'node_modules'), join(basedir, 'old', 'node_modules'))
      .then(() => rename(join(basedir, 'package.json'), join(basedir, 'old', 'package.json'))
      .then(() => rename(join(basedir, 'version.txt'), join(basedir, 'old', 'version.txt'))
      .catch(err => null)
      .then(() => rename(join(basedir, 'next', 'app'), join(basedir, 'app'))
      .then(() => rename(join(basedir, 'next', 'node_modules'), join(basedir, 'node_modules'))
      .then(() => rename(join(basedir, 'next', 'package.json'), join(basedir, 'package.json'))
      .then(() => rename(join(basedir, 'next', 'package.json'), join(basedir, 'package.json'))
      .then(() => rename(join(basedir, 'next', 'version.txt'), join(basedir, 'version.txt'))
    }
  })
  .then(() => {
